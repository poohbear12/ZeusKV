...
//        System.out.println(configToString(zeusConfig));
  ...
  public String configToString(ZeusConfig config) {
    ...
    if (config.getNodes() == null || config.getNodes().isEmpty()) {
      ...
      for (int i = 0; i < config.getNodes().size(); i++) {
        ...
        sb.append("    Failure Detection Timeout: ").append(nodeConfig.getFailureDetectionTimeout())
            .append("\n");
  ...
  private void fillDefaultNodeValues(NodeConfig nodeConfig) {
    if (nodeConfig.getName() == null) {
      nodeConfig.setName("default_node");
    }
    if (nodeConfig.getAddr() == null) {
      nodeConfig.setAddr("localhost");
    }
    if (nodeConfig.getPort() <= 0) {
      nodeConfig.setPort(6379); // 端口号保持正数
    }
    if (nodeConfig.getType() == null) {
      nodeConfig.setType(NodeType.SINGLE);
    }
    if (nodeConfig.getClusterId() == 0) {
      nodeConfig.setClusterId(-1);
    }
    if (nodeConfig.getHeartbeatInterval() <= 0) {
      nodeConfig.setHeartbeatInterval(-1);
    }
    if (nodeConfig.getFailureDetectionTimeout() <= 0) {
      nodeConfig.setFailureDetectionTimeout(-1);
    }
    if (nodeConfig.getMaxConnections() <= 0) {
      nodeConfig.setMaxConnections(-1);
    }
  }
...
//        if (aof.isEnable()) aof.setEnable(true); // 默认启用
    if (aof.getFilename() == null) {
      aof.setFilename("zeus.aof");
    }
    if (aof.getPreallocateSize() <= 0) {
      aof.setPreallocateSize(-1);
    }
    if (aof.getFlushInterval() <= 0) {
      aof.setFlushInterval(-1);
    }
    if (aof.getLargeCmdThreshold() <= 0) {
      aof.setLargeCmdThreshold(-1);
    }
    if (aof.getBackpressureThreshold() <= 0) {
      aof.setBackpressureThreshold(-1);
    }
    if (aof.getQueueSize() <= 0) {
      aof.setQueueSize(-1);
    }
    if (aof.getMinBatchSize() <= 0) {
      aof.setMinBatchSize(-1);
    }
    if (aof.getMaxBatchSize() <= 0) {
      aof.setMaxBatchSize(-1);
    }
    if (aof.getMinBatchTimeout() <= 0) {
      aof.setMinBatchTimeout(-1);
    }
    if (aof.getMaxBatchTimeout() <= 0) {
      aof.setMaxBatchTimeout(-1);
    }
  }
